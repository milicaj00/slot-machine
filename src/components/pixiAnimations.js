...
const draw = () => {
    ...
    for (let i = 0; i < startMatrix[0]?.length; i++) {
        ...
        for (let j = 0; j < startMatrix.length; j++) {
            ...
            const element = new PIXI.Sprite(PIXI.Texture.from(startMatrix[j][i]));
...
app.ticker.add(delta => {
    columns.forEach((column, j) => {
        ...
        column.elements.forEach((el, i) => {
            ...
            el.y = ((column.position + i) % column.elements.length) * SYMBOL_SIZE - SYMBOL_SIZE;

            if (el.y < 0 && prevy > SYMBOL_SIZE && !choosen) {
                el.texture = PIXI.Texture.from(
                    // textures[Math.floor(Math.random() * textures.length)]
                    startMatrix[Math.floor(Math.random() * 4)][Math.floor(Math.random() * 5)]);
